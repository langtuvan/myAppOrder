...
import {
  useOrderList,
  ExportStatusOptions as statusOptions,
} from "@/hooks/useOrders";

export default function ProductPage() {
  const onSelect = (id: string) => {
    router.push(paths.dashboard.orders.export(id));
  };
  const {
    router,
    selectedDate,
    ...
    setCurrentStatus,
    dataFiltered,
  } = useOrderList({ statusOptions, defaultStatus: OrderStatus.CONFIRMED });
  return (
    <>
      <div className="flex w-full flex-wrap items-end justify-between gap-4 border-b border-zinc-950/10 pb-6 dark:border-white/10">
        <Heading>Export List</Heading>
      </div>
